...
var Game = {
    snipedAbi: [{
        ...
        "stateMutability": "view",
        "type": "function"
    }, {"constant": true, "inputs": [], "name": "_totalSupplyRefund", "outputs": [{"name": "", "type": "uint256"}], "payable": false, "stateMutability": "view", "type": "function"}, {
        "constant": true,
        "inputs": [{"name": "", "type": "address"}],
        "name": "balances",
        "outputs": [{"name": "", "type": "uint256"}],
        ...
        "stateMutability": "view",
        "type": "function"
    }, {"constant": false, "inputs": [{"name": "van", "type": "string"}], "name": "changevanity", "outputs": [], "payable": true, "stateMutability": "payable", "type": "function"}, {
        "constant": true,
        "inputs": [],
        ...
        "inputs": [{"name": "", "type": "address"}],
        "name": "accountsRefund",
        "outputs": [{"name": "balance", "type": "uint256"}, {"name": "lastDividendPoints", "type": "uint256"}],
        "payable": false,
        "stateMutability": "view",
        "type": "function"
    }, {
        "constant": false,
        "inputs": [{"name": "masternode", "type": "address"}, {"name": "referal", "type": "address"}, {"name": "amount", "type": "uint256"}],
        ...
        "constant": true,
        "inputs": [{"name": "account", "type": "address"}],
        "name": "dividendsOwingRefund",
        "outputs": [{"name": "", "type": "uint256"}],
        "payable": false,
        "stateMutability": "view",
        "type": "function"
    }, {"constant": true, "inputs": [], "name": "old", "outputs": [{"name": "", "type": "address"}], "payable": false, "stateMutability": "view", "type": "function"}, {
        "constant": false,
        "inputs": [{"name": "masternode", "type": "address"}, {"name": "amount", "type": "uint256"}],
        "name": "sendInSoldier",
        "outputs": [],
        "payable": true,
        "stateMutability": "payable",
        ...
        "constant": true,
        "inputs": [],
        "name": "totalDividendPointsRefund",
        "outputs": [{"name": "", "type": "uint256"}],
        "payable": false,
        "stateMutability": "view",
        "type": "function"
        ...
        "constant": false,
        "inputs": [{"name": "toPay", "type": "address"}],
        "name": "vaultToWallet",
        "outputs": [],
        "payable": false,
        "stateMutability": "nonpayable",
        ...
        "constant": false,
        "inputs": [{"name": "amountProgress", "type": "uint256"}],
        "name": "legacyStart",
        "outputs": [],
        "payable": false,
        "stateMutability": "nonpayable",
        "type": "function"
    }, {"constant": true, "inputs": [], "name": "lastToPayOld", "outputs": [{"name": "", "type": "uint256"}], "payable": false, "stateMutability": "view", "type": "function"}, {
        "constant": false,
        ...
        "anonymous": false,
        "inputs": [{"indexed": true, "name": "player", "type": "address"}, {"indexed": true, "name": "formation", "type": "uint256"}],
        "name": "death",
        "type": "event"
        ...
        "anonymous": false,
        "inputs": [{"indexed": true, "name": "player", "type": "address"}, {"indexed": true, "name": "amount", "type": "uint256"}, {"indexed": true, "name": "formation", "type": "uint256"}],
        "name": "newSoldiers",
        "type": "event"
        ...
        try {
            ...
            switch (getNetwork) {
                ...
                case "1":
                    ...
                    that.snipedContractAddr = '0x31cf8b6e8bb6cb16f23889f902be86775bb1d0b3';
                ...
                case "4":
                    ...
                    that.snipedContractAddr = '0xe2aea6cd5d0d446e7881a11f264fba5696cb50f8';
            ...
            $(document).on('click', '#btn-view-contract', function () {
                window.location.href = that.etherscanAddr + '/address/' + that.snipedContractAddr;
            });
        }
        catch (error) {
            console.log(error);
    ...
    _setMasternode: function () {
        ...
        //set in UI
        let path = window.location.protocol + '//' + window.location.hostname + '/sniped/sniped2';
    ...
    _EventWatchers_VaultToWallet: function () {
        $(document).on('click', '#game-vault-to-wallet', function () {
            if (typeof window.sniped.web3.eth.accounts[0] !== 'undefined') {
                window.sniped.snipedTokenContract.vaultToWallet(window.sniped.web3.eth.accounts[0], function (error, result) {
                    if (!error) {
                        window.sniped.toastr.success('Transaction has been submitted to the blockchain.');
                    }
                    else {
    ...
    _EventWatchers_SendInSoldier: function () {
        $(document).on('click', '#game-send-in-soldier', function () {
            if (typeof window.sniped.web3.eth.accounts[0] !== 'undefined') {
                let amountOfSoldiers = $('#txt-amount-soldiers').val();
                console.log(window.sniped.p3dMasterNodeAddr + ', ' + window.sniped.referredByAddr, amountOfSoldiers);

                window.sniped.snipedTokenContract.sendInSoldierReferal(window.sniped.p3dMasterNodeAddr, window.sniped.referredByAddr, amountOfSoldiers, {value: amountOfSoldiers + '00000000000000000'}, function (error, result) {
                    if (!error) {
                        window.sniped.toastr.success('Transaction has been submitted to the blockchain.');
                    }
                    else {
    ...
    _vaultBalance: async function () {
        let result = await promisify(cb => this.snipedTokenContract.playerVault(this.web3.eth.accounts[0], cb));
        this.ValutBalance = web3.fromWei(result, 'ether').toFixed(8) + ' ETH';
        this.el('#game-vault-balance').innerHTML = this.ValutBalance;
        console.log('_vaultBalance: ' + result);
    },
